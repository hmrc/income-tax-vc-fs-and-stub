...
case class CrystallisationStatus(
 ...
 // private def taxYearField: Option[String] = if (is1896) None else Some(taxYear.rangeLong)
  ...
  def makeOverwriteJson: JsValue = {
    Json.arr(
      ...
      //  "calculationType"      -> calculationTypeField   to change to "finalDeclaration?"
      ) ++
     // Json.obj(
 ...
 // }
  ...
  def makeOverwriteDataModel: DataModel =
    DataModel(
      _id = url,
     // schemaId = getSchemaIdValue, to chnage to schema id name "getCalculationListDetailsSuccessHip"
      method = "GET",
