...
package uk.gov.hmrc.incometaxvcfsandstub.testUtils
...
trait TestSupport
  ...
  def readFile(path: String): String = {
    // allow old calls like "test/resources/x.json" by stripping the prefix
    val normalised = path.stripPrefix("test/resources/").stripPrefix("/")

    val stream = Option(getClass.getClassLoader.getResourceAsStream(normalised))
      .getOrElse(throw new java.io.FileNotFoundException(s"Classpath resource not found: $normalised (original: $path)"))
