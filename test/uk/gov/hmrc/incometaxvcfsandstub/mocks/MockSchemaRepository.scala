...
package uk.gov.hmrc.incometaxvcfsandstub.mocks

import com.mongodb.client.result.{DeleteResult, InsertOneResult}
import uk.gov.hmrc.incometaxvcfsandstub.models.SchemaModel
import org.mockito.ArgumentMatchers
import org.mockito.Mockito._
import org.mockito.stubbing.OngoingStubbing
import org.scalatestplus.mockito.MockitoSugar
import uk.gov.hmrc.incometaxvcfsandstub.repositories.SchemaRepository
...
trait MockSchemaRepository extends TestSupport with MockitoSugar {
  ...
  def setupMockAddSchema(model: SchemaModel)(response: InsertOneResult): OngoingStubbing[Future[InsertOneResult]] =
    when(mockSchemaRepository.addEntry(ArgumentMatchers.eq(model)))
