...
import uk.gov.hmrc.incometaxvcfsandstub.adts._
  ...
  ".validateJsonAgainstYamlSchema()" when {
    ...
    "API-1878" should {
      ...
      "return Left ValidationFailure when json contains multiple invalid data values for date and status field" in {
        ...
        result match {
          case Left(ValidationFailure(msg)) =>
            msg should include("instance value (\"123456\") not found in enum")
            msg should include("possible values")
            msg should include("string \"ABC\" is invalid against requested date format(s)")
            msg should include("yyyy-MM-dd'T'HH:mm:ssZ")
          case other =>
            fail(s"Expected Left(ValidationFailure), got: $other")
        }
      }
